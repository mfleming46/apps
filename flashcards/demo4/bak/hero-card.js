/* ===== hero-card.js — standalone summary hero (uses external medal SVGs) ===== */
/* global HeroCard */
(function (global) {
  function chooseBadge(pct, attempted, total, early) {
    if (!early && attempted === total) {
      ...
      else                return ["teal",   "complete"]; // completed but <80
    }
  ...
  function render(mountEl, stats, opts) {
    ...
    const o = Object.assign({ size:"", images:{} }, opts || {});
    ...
    // image paths (defaults assume svg files live next to this JS)
    ...
    if (o.size === "spacious") hero.classList.add("hc-hero--spacious");
    if (o.size === "compact")  hero.classList.add("hc-hero--compact");
    ...
    // allow parent to listen for restart
